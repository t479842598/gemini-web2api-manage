...
- **真实多轮会话续接**：实测确认 payload `inner[2]` 的前两位放 `[conversation_id, response_id]` 时，Gemini 会真正在服务端续接会话（实测第二轮正确复述出"Xiaoming / 25 岁"），不再需要把整段历史压平成一条消息。用法：把上一次响应里的 `gemini_conversation_id` 与 `gemini_response_id` 原样放回请求体即可。
  - **仅在配置了 Cookie 时启用**：匿名下带真实 ID 实测返回 `1096`，若不加守卫等于把一个本来可用的请求变成报错。
  - **失败自动降级**：命中 `1096/1097/1003` 且本次确实带了续接 ID 时，清掉 ID 按无状态重试一次（流式仅在尚未输出任何内容时才重试，避免重复输出）。续接是优化，不该因为它让请求失败。
- **Cookie 认证态显式探测**（`auth_state.py`）：Cookie 会被 Google 悄悄降级为匿名，此前服务不报错、只是模型变回 lite，用户完全无从得知。
  - 判据（实测确立，比看 `served_model` 可靠）：带 Cookie 但**不带 `at`** 发一次请求 —— `HTTP 400` = Google 在校验 `at` ⇒ 登录态有效；`HTTP 200` = 不校验 ⇒ 已被当作匿名。
  - 状态经 TTL 缓存（`auth_probe_ttl_sec`，默认 600 秒），从 `authenticated` 翻到 `anonymous` 时记一条明确日志。
  - `/health` 与 `/admin/api/status` 暴露只读快照；`GET /admin/api/auth-probe`（管理员鉴权）强制重探。轮询管理台不会消耗 Google 请求。

### 修复

- **`1003` 的提示文案是错的**。上一版把它解释成"Cookie 失效/被降级"，但实测在认证态明确有效时（`served=3.1 Pro`、缺 `at` 返回 400）带图请求**仍然**返回 1003 —— 与登录态无关。进一步实验表明 1003 是通用的"引用不被接受"：伪造的会话续接 ID 与附件引用都会触发它。现按实测改写，并补充 1096 的准确说明。
- **`1099` 纳入瞬时错误自动重试**。实测推送 Cookie 后首个请求报 1099、紧接着 6/6 全成功，属认证握手抖动；此前它直接冒泡成 502，表现为"推送成功但用不了"。

### 实测结论（本轮带 Cookie 验证）

| 项 | 结论 |
|---|---|
| Cookie 能否解锁真实模型 | **能**。`served_model` 出现 `3.1 Pro` / `3.6 Flash`（匿名恒为 `3.5 Flash-Lite`） |
| `at` 是否必需 | 登录态下**必需**（缺 `at` → HTTP 400）；匿名不需要。**body 与 query 两个位置都接受** |
| `SAPISIDHASH` 是否必需 | **不必需**（去掉 `Authorization` 照样成功） |
| 会话续接 | 带有效 Cookie **可用**（`inner[2]=[c_id, r_id]`）；匿名返回 1096 |
| 多模态附件 | **仍不可用**，1003 与登录态无关，属上游 file binding 未打通 |
| Cookie 存活窗口 | 约 **20–35 分钟**（数十次请求）后被降级为匿名；根因未定（PSIDTS 轮转 vs 数据中心 IP 风控），已排除配额耗尽 |
| mode 档位 | mode 4/5/6 稳定落 lite 档（2.2s）；mode 1/2/3 落 Pro/Flash（5–95s）—— 现有模型表映射与实际不符，待重排 |
